# 206267

**Source**: [opensourcebrain](https://github.com/OpenSourceBrain/206267)
**Standard**: other
**Authors**: Open Source Brain

## Description

Cerebellum granule cell FHF (Dover et al. 2016)


## Usage

This model was auto-generated from the opensourcebrain repository.

```yaml
# In a space.yaml wiring file:
models:
  - repo: Biosimulant/models
    alias: model
    manifest_path: models/neuroscience-other-cerebellum-granule-cell-fhf-osb206267-model/model.yaml
```

## Tags

other, opensourcebrain, auto-generated, neuroscience
