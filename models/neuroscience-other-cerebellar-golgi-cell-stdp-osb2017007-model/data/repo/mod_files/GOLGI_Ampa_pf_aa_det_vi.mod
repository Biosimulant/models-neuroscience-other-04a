...
FUNCTION eta(ci (mM)) { : when ci is 0, inv has to be 3 hours.
    ...
    P1=100	
    P2=0.002	: There was a slip in the paper, which says P2=P1/1e-4
    P4=1000
    P3=3	: Cube, directly multiplying, see below.
    
    ci=(ci-1e-4)*1e3 	: The function takes uM, and we get mM.
    
    inv=P4 + P1/(P2+(ci*ci*ci*ci)) :As P3 is 3, set ci^P3 as ci*ci*ci.
    eta=1/inv
}	
   ...
   if (flag == 0) { 
		...
		if (!on) {
			...
			if (tau_facil > 0) { 
				...
				u = u + U * (1+30*u) * (exp(-5*u)- exp(-5))    :U = ((U * w)/0.5)
			} else { u = U }
